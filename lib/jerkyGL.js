...
export function buildAdSpend(glRows, coaRows) {
  glRows = Array.isArray(glRows) ? glRows : []
  // advertising expense accounts = CoA names under "Advertising/Marketing"
  let adLeaves = new Set()
  for (const c of (coaRows || [])) {
    if (/^Advertising\/Marketing/.test(c.account)) adLeaves.add(c.leaf || leafOf(c.account))
  }
  if (!adLeaves.size) adLeaves = new Set(['Advertising/Marketing', 'Graphics Design', 'Photography'])
  ...
  const payee = (name, desc) => {
    ...
    const d = String(desc || '')
    let m = /payment to (.+?)(?:\s+[A-Z0-9]{6,}\b.*)?$/i.exec(d)
    if (m && m[1]) return m[1].replace(/\s+[A-Z0-9]{6,}$/i, '').trim()
    m = /IND NAME:\s*([A-Za-z][A-Za-z .&'\-]+?)(?:\s{2,}|$)/.exec(d)
    if (m) return m[1].trim()
    m = /ORIG CO NAME:\s*([A-Za-z][A-Za-z .&'\-]+?)(?:\s{2,}|ORIG|$)/.exec(d)
    if (m) return m[1].trim()
  ...
  return { total: round2(total), byCategory: toArr(byCat), vendors: toArr(byVendor), lastDate }
}
