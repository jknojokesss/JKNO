...
export async function runCloverSync({ start, end } = {}) {
  ...
  try { await supabaseAdmin.from('sync_log').insert({ source: 'clover', ok: true, rows: inserted, message: `${result.window.start} → ${result.window.end}` }) } catch (_) {}
  return result
  } catch (e) {
    try { await supabaseAdmin.from('sync_log').insert({ source: 'clover', ok: false, message: String(e.message || e).slice(0, 300) }) } catch (_) {}
    throw e
  }
}
