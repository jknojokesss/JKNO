...
export async function fetchArRefs(env, accessToken, realmId) {
  const page = async (entity, fields) => {
    const out = []
    for (let start = 1; ; start += 1000) {
      const q = `select ${fields} from ${entity} where Active = true startposition ${start} maxresults 1000`
      const json = await qboGet(env, accessToken, `/v3/company/${realmId}/query?query=${encodeURIComponent(q)}`)
  ...
  const [customers, items] = await Promise.all([
    page('Customer', 'Id, DisplayName, PrimaryEmailAddr'),
    page('Item', 'Id, Name, Type, UnitPrice'),
  ])
  return {
 ...
 */
export function buildInvoice({ customerId, txnDate, dueDate, emailTo, lines }, refs) {
  const errors = []
  ...
  return { payload, errors, total, summary: { customer: cust ? cust.name : null, email: email || null, total } }
}
