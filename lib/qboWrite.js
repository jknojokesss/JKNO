 ...
 */
export function buildInvoice({ customerId, customerName, txnDate, dueDate, memo, billEmail, lines, docNumber }) {
  const errors = []
  if (!customerId) errors.push('No QuickBooks customer.')
  if (!txnDate || !/^\d{4}-\d{2}-\d{2}$/.test(txnDate)) errors.push('Date must be YYYY-MM-DD.')
  if (docNumber && String(docNumber).length > DOCNUMBER_MAX) errors.push(`Invoice number "${docNumber}" is too long (max ${DOCNUMBER_MAX}).`)
  const Line = (lines || []).map((l, i) => {
...
/** The company's configured invoice-email subject + body (what QuickBooks sends). */
export async function fetchInvoiceEmailPrefs(env, accessToken, realmId) {
  const json = await qboGet(env, accessToken, `/v3/company/${realmId}/query?query=${encodeURIComponent('select * from Preferences')}&minorversion=${MINOR_VERSION}`)
  const prefs = (json.QueryResponse && json.QueryResponse.Preferences && json.QueryResponse.Preferences[0]) || null
  if (!prefs) return null
  const em = (prefs.EmailMessagesPrefs && prefs.EmailMessagesPrefs.InvoiceMessage) || null
  const sales = prefs.SalesFormsPrefs || null
  return {
    subject: em && em.Subject ? em.Subject : null,
    message: em && em.Message ? em.Message : null,
    customerMessage: sales && sales.DefaultCustomerMessage ? sales.DefaultCustomerMessage : null,
    // when ON, QuickBooks will NOT auto-number an API invoice — we must supply DocNumber
    customTxnNumbers: !!(sales && sales.CustomTxnNumbers),
  }
}

/**
 * The next sequential invoice number, from the highest numeric DocNumber in
 * recent invoices (preserving any prefix + zero-padding). null if none parse.
 */
export async function nextInvoiceNumber(env, accessToken, realmId) {
  const q = `select DocNumber, TxnDate from Invoice orderby TxnDate desc maxresults 100`
  const json = await qboGet(env, accessToken, `/v3/company/${realmId}/query?query=${encodeURIComponent(q)}&minorversion=${MINOR_VERSION}`)
  const invs = (json.QueryResponse && json.QueryResponse.Invoice) || []
  let best = 0, prefix = '', width = 0
  for (const inv of invs) {
    const m = /^(\D*?)(\d+)$/.exec(String(inv.DocNumber || '').trim())
    if (!m) continue
    const n = parseInt(m[2], 10)
    if (n > best) { best = n; prefix = m[1]; width = m[2].length }
  }
  if (!best) return null
  return prefix + String(best + 1).padStart(width, '0')
}

/** All of a customer's invoices (optionally since a date), for reconciling A/R. */
export async function fetchInvoicesForCustomer(env, accessToken, realmId, customerId, sinceDate) {
  const out = []
  for (let start = 1; ; start += 1000) {
    const where = `where CustomerRef = '${customerId}'` + (sinceDate ? ` and TxnDate >= '${sinceDate}'` : '')
    const q = `select * from Invoice ${where} startposition ${start} maxresults 1000`
    const json = await qboGet(env, accessToken, `/v3/company/${realmId}/query?query=${encodeURIComponent(q)}&minorversion=${MINOR_VERSION}`)
...
/** Map a raw QBO Invoice to our store_invoices row shape (source of truth = QB). */
export function invoiceToRow(inv, partnerId) {
  const line = (inv.Line || []).find((l) => l.DetailType === 'SalesItemLineDetail')
  const d = (line && line.SalesItemLineDetail) || {}
  const balance = Number(inv.Balance || 0)
  return {
    partner_id: partnerId || null,
    inv_date: inv.TxnDate || null,
    due_date: inv.DueDate || null,
    units: d.Qty != null ? Math.round(Number(d.Qty)) : null,
    unit_price: d.UnitPrice != null ? Number(d.UnitPrice) : null,
    amount: Number(inv.TotalAmt) || 0,
    description: (line && line.Description) || null,
    line_item: (d.ItemRef && d.ItemRef.name) || null,
    status: balance > 0.005 ? 'unpaid' : 'paid',
    paid_date: balance > 0.005 ? null : (inv.TxnDate || null),
...
/** Read one invoice (for its CustomerRef, Balance, and current SyncToken). */
export async function fetchInvoice(env, accessToken, realmId, id) {
  const json = await qboGet(env, accessToken, `/v3/company/${realmId}/invoice/${id}?minorversion=${MINOR_VERSION}`)
  return json.Invoice || null
}

/** A customer payment that fully applies to one invoice. */
export function buildPayment({ customerId, invoiceId, amount, txnDate }) {
  return {
    CustomerRef: { value: String(customerId) },
    TotalAmt: round2(amount),
    TxnDate: txnDate,
    Line: [{ Amount: round2(amount), LinkedTxn: [{ TxnId: String(invoiceId), TxnType: 'Invoice' }] }],
  }
}
export function postPayment(env, accessToken, realmId, payload, requestId) {
  return qboPost(env, accessToken, realmId, 'payment', payload, { requestId })
}
export async function fetchPayment(env, accessToken, realmId, id) {
  const json = await qboGet(env, accessToken, `/v3/company/${realmId}/payment/${id}?minorversion=${MINOR_VERSION}`)
  return json.Payment || null
}
export function deletePayment(env, accessToken, realmId, id, syncToken) {
  return qboPost(env, accessToken, realmId, 'payment', { Id: String(id), SyncToken: String(syncToken) }, { operation: 'delete' })
}
