...
export async function fetchGeneralLedgerRows(env, accessToken, realmId, { months = 3 } = {}) {
  ...
  for (const m of monthsList) {
    ...
    const cols = (report.Columns && report.Columns.Column) || []
    const idxOf = (key) => cols.findIndex((c) => (c.MetaData || []).some((md) => md.Name === 'ColKey' && md.Value === key))
    ...
    const walk = (list, account) => {
      for (const row of list || []) {
        ...
        if (row.type === 'Data' && row.ColData) {
          ...
          const amount = parseFloat(val(cd, iAmt))
          const txnDate = val(cd, iDate)
          if (!txnDate || isNaN(amount)) continue // skip beginning-balance/total rows
          out.push({
            month: m.key,
            txn_date: txnDate,
  ...
  return { rows: out, months: monthsList.map((m) => m.key) }
}
