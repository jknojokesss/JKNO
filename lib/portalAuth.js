...
export async function isPortalClient(clientSlug) {
  ...
  const { data, error } = await supabaseAdmin
    .from('portal_users').select('email, protect_from_admin').eq('client_slug', clientSlug).limit(1).maybeSingle()
  if (error) throw new Error(`Could not check portal ownership: ${error.message}`)
  // protect_from_admin=false is for our own books used as a portal self-test:
  // the portal login exists so the client-facing flow can be exercised on
  // data we own, and it must not lock us out of our own admin tools.
  return !!data && data.protect_from_admin !== false
}
