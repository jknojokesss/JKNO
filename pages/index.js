...
import HomeFeatures, { HomeRotator } from '../components/HomeFeatures'
import { BOOKING_URL } from '../lib/marketing'

export default function Landing() {
  const [introPlaying, setIntroPlaying] = useState(false)
  const [heroLive, setHeroLive] = useState(false)
  const [focusFeatures, setFocusFeatures] = useState(0)
  ...
  const handleWhatWeBuild = () => {
    setFocusFeatures((n) => n + 1)
  ...
  return (
    ...
    >
      ...
      <div className={`m-home${heroLive ? ' m-home--live' : ''}`}>
        <section className="m-dark-band">
          <div className="m-wrap m-band">
            <div className="m-band__hero">
              ...
              <button type="button" className="m-btn m-btn--primary m-btn--pop" onClick={handleWhatWeBuild}>
                What we build ↓
              </button>
            </div>

            <HomeFeatures live={heroLive} focusPulse={focusFeatures} />
          </div>
        </section>
